...
class Goodwe extends IPSModule
{
        ...
        foreach ($selectedRegisters as $register) {
            if (isset($register['address'], $register['type'], $register['scale'])) {
                ...
                if ($varId !== false) {
                    ...
                    $this->SendDebug("RequestRead", "Register {$register['address']} aktualisiert: $value", 0);
                } else {
    ...
    private function ReadRegister(int $address, string $type, float $scale)
    {
        ...
        $this->SendDebug("ReadRegister", "Register $address: Originalwert=$value, Skaliert=$scaledValue", 0);
        return $scaledValue;
    }
