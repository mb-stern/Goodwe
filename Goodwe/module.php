...
class Goodwe extends IPSModule
{
    ...
    {
        // --- Reentrancy-Guard + Timer-Pause ---
        $sem = get_class($this) . '_WR_' . $this->InstanceID;
        if (!IPS_SemaphoreEnter($sem, 0)) {
            $this->SendDebug('TimerWR', 'Übersprungen: Vorheriger Durchlauf läuft noch.', 0);
            return;
        }

        // aktuellen Intervall merken und Timer pausieren
        $intervalMs = max(0, (int)$this->ReadPropertyInteger('PollIntervalWR') * 1000);
        $this->SetTimerInterval('TimerWR', 0);

        try {
            // ===== ab hier dein bisheriger Body (unverändert) =====
            $selectedRegisters = json_decode($this->ReadPropertyString("SelectedRegisters"), true);
            if (!is_array($selectedRegisters)) {
            ...
            $this->CalculateMaxPower();
            // ===== dein bisheriger Body Ende =====

        } catch (Throwable $e) {
            $this->SendDebug('TimerWR', 'Fehler: ' . $e->getMessage(), 0);
            $this->LogMessage('Goodwe/TimerWR: ' . $e->getMessage());
        } finally {
            // Timer zuverlässig wiederherstellen + Lock freigeben
            if ($intervalMs > 0) {
                $this->SetTimerInterval('TimerWR', $intervalMs);
            }
    ...
    {
        // --- Reentrancy-Guard + Timer-Pause ---
        $sem = get_class($this) . '_WB_' . $this->InstanceID;
        if (!IPS_SemaphoreEnter($sem, 0)) {
            $this->SendDebug('TimerWB', 'Übersprungen: Vorheriger Durchlauf läuft noch.', 0);
            return;
        }

        $intervalMs = max(0, (int)$this->ReadPropertyInteger('PollIntervalWB') * 1000);
        ...
        try {
            ...
            $this->SendDebug('TimerWB', 'Fehler: ' . $e->getMessage(), 0);
            $this->LogMessage('Goodwe/TimerWB: ' . $e->getMessage());
        } finally {
            if ($intervalMs > 0) {
                $this->SetTimerInterval('TimerWB', $intervalMs);
            }
            IPS_SemaphoreLeave($sem);
        }
    }
