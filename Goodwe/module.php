...
class Goodwe extends IPSModule
{
    public function Create()
    {
        ...
        $this->RegisterPropertyInteger("Poller", 60); // Polling-Intervall in Sekunden
    ...
    public function ApplyChanges()
    {
        ...
        // Timer für Polling setzen
        $pollInterval = $this->ReadPropertyInteger("Poller");
        ...
        foreach ($selectedRegisters as $register) {
            if (isset($register['address'], $register['type'], $register['scale'])) {
                ...
                if ($response === false || strlen($response) < (2 * $quantity + 2)) {
                    $this->SendDebug("Error", "Keine oder unvollständige Antwort für Register {$register['address']}", 0);
                    continue;
                }
                ...
                // Werte interpretieren basierend auf Typ
                ...
                if ($varId !== false) {
                    ...
                    $this->SendDebug("RequestRead", "Register {$register['address']} aktualisiert: $scaledValue", 0);
                } else {
                    $this->SendDebug("RequestRead", "Variable für Register {$register['address']} existiert nicht.", 0);
                }
            } else {
                $this->SendDebug("RequestRead", "Ungültiges Register: " . json_encode($register), 0);
            }
        }
    }
