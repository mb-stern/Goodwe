...
class Goodwe extends IPSModule
{
    ...
    public function FetchInverterData()
    {
        ...
        // Masterliste für Register
        $masterIndex = [];
        foreach ($this->GetRegisters() as $mr) {
            $masterIndex[(string)$mr['address']] = $mr;
        }

        // Hier sammeln wir alle geänderten Register für eine kompakte Debug-Zeile
        $updatedRegisters = [];

        foreach ($selectedRegisters as &$r) {
            // Einträge können als JSON-String vorliegen → dekodieren
            if (is_string($r)) {
                $tmp = json_decode($r, true);
                if (is_array($tmp)) {
                    $r = $tmp;
            ...
            // Nur selektierte Register
            if (isset($r['selected']) && !$r['selected']) {
                continue;
            }

            // Alt: 'addr' → neu: 'address'
            if (!isset($r['address']) && isset($r['addr'])) {
                $r['address'] = $r['addr'];
            }

            // Sonderfall: address als JSON-String (z.B. {"address":36019,...})
            if (isset($r['address']) && is_string($r['address']) && str_starts_with(trim($r['address']), "{")) {
                $decoded = json_decode($r['address'], true);
                if (is_array($decoded)) {
                    $r = array_replace($r, $decoded);
            ...
            try {
                ...
                switch ($var['VariableType']) {
                    ...
                    case VARIABLETYPE_FLOAT:
                        // Anzahl Nachkommastellen aus Scale ableiten
                ...
                if ($current !== $scaledValue) {
                    ...
                    $updatedRegisters[$ident] = $scaledValue;
                }
        ...
        // Zusammengefasste Debug-Zeile für alle geänderten Register
        if (!empty($updatedRegisters)) {
            $this->SendDebug(
                "FetchInverterData",
                "Aktualisierte Register: " . json_encode($updatedRegisters),
                0
            );
        } else {
            $this->SendDebug("FetchInverterData", "Keine Registeränderungen in diesem Zyklus.", 0);
        }

        // Deine zusätzliche Berechnung wie bisher
        $this->CalculateMaxPower();
    }
    ...
    public function FetchWallboxData()
    {
        ...
        try {
            ...
            $apiResponseV4 = $this->GoodweFetchData($serial);
            if (!$apiResponseV4) {
                $this->SendDebug("FetchWallboxData", "API-Datenabruf (v4) fehlgeschlagen.", 0);
                return;
            }

            $dataV4 = json_decode($apiResponseV4, true);
            if (!isset($dataV4['data']) || !is_array($dataV4['data'])) {
                $this->SendDebug("FetchWallboxData", "Keine gültigen 'data'-Daten im API-Response (v4).", 0);
                return;
            }

            $apiDataV4 = $dataV4['data'];

            // **Das ist der Debug, den du vorher hattest: kompletter data-Block**
            $this->SendDebug(
                "FetchWallboxData",
                "Rohdaten (v4 GetEvChargerAloneViewBySn): " . json_encode($apiDataV4),
                0
            );

            $apiResponseV3 = $this->GoodweFetchChargeInfo($serial);
            if ($apiResponseV3) {
                $dataV3 = json_decode($apiResponseV3, true);
                if (isset($dataV3['data']) && is_array($dataV3['data'])) {
                    $apiDataV3 = $dataV3['data'];

                    // Einheitlicher Debug-Stil für den zweiten Endpunkt
                    $this->SendDebug(
                        "FetchWallboxData",
                        "Rohdaten (v3 GetCurrentChargeinfo): " . json_encode($apiDataV3),
                        0
                    );

                } else {
                    $this->SendDebug("FetchWallboxData", "v3-Response enthält keinen gültigen 'data'-Block.", 0);
                }
            } else {
                $this->SendDebug("FetchWallboxData", "API-Datenabruf (v3) fehlgeschlagen – v3-Rohdaten nicht verfügbar.", 0);
            }
            ...
            foreach ($apiDataV4 as $key => $value) {
                $ident = "WB_" . $key;
                $varID = @$this->GetIDForIdent($ident);
                ...
                // Ist-Leistung: API liefert kW → intern W
                if ($key === 'power') {
                ...
                // workstate → WB_Charging (mit Pending-/Block-Logik)
                if ($key === "workstate") {
                    ...
                    if (!$isPendingCharging && !$isBlockedCharging) {
                        ...
                        $this->SendDebug(
                            "FetchWallboxData",
                            "WB_Charging aus API (v4) gesetzt: " . ($chargingState ? "true" : "false"),
                            0
                        );
                    } elseif ($isBlockedCharging) {
                        $this->SendDebug(
                            "FetchWallboxData",
                            "WB_Charging nicht aktualisiert – blockiert bis " . date('H:i:s', $holdUntil),
                            0
                        );
                ...
                // chargeMode (v4) → WB_ChargeMode (Soll), wenn nicht pending/blockiert
                if ($key === "chargeMode") {
                    ...
                    if (!$isPendingMode && !$isBlockedMode) {
                        ...
                        $this->SendDebug(
                            "FetchWallboxData",
                            "WB_ChargeMode aus API (v4) gesetzt: " . (int)$value,
                            0
                        );
                    }
                }

                // WB_ChargePower bleibt write-only (API liefert keinen gesetzten Sollwert)
            }
    ...
    private function GoodweFetchChargeInfo(string $serial): ?string
    {
        $this->SendDebug("GoodweFetchChargeInfo", "Starte API-Datenabruf (v3 GetCurrentChargeinfo) für Seriennummer: $serial", 0);

        $apiEndpoint = "/v3/EvCharger/GetCurrentChargeinfo";

        // Body analog zu GoodweFetchData, nur mit anderem api-Endpunkt
        ...
        if ($httpCode !== 200 || !$response) {
            $this->SendDebug("GoodweFetchChargeInfo", "API-Datenabruf (v3) fehlgeschlagen. HTTP-Code: $httpCode, Antwort: $response", 0);
            return null;
        }

        $this->SendDebug("GoodweFetchChargeInfo", "API-Daten (v3) erfolgreich abgerufen. Antwort: $response", 0);
        return $response;
    }

    private function GoodweFetchData(string $serial): ?string
    {
        $endpoint = "/v4/EvCharger/GetEvChargerAloneViewBySn";

        $this->SendDebug(
            "GoodweFetchData",
            "Hole Wallbox-Daten von {$endpoint} für SN={$serial}",
            0
        );
        ...
        if ($code !== 200 || !$response) {
            $this->SendDebug(
                "GoodweFetchData",
                "Fehler HTTP={$code}, Response=" . substr((string)$response, 0, 300),
                0
            );
            return null;
        }
        ...
        $this->SendDebug(
            "GoodweFetchData",
            "RAW Wallbox-Daten: " . substr($response, 0, 500),
            0
        );
    ...
    private function SendWallboxRequest(array $data, string $endpoint): ?array
    {
        ...
        if (empty($email) || empty($password)) {
            $this->SendDebug("SendWallboxRequest", "Abbruch: Benutzername oder Passwort fehlt.", 0);
            return null;
        }

        // Login
        if (!$this->LoginToWallbox($email, $password)) {
            $this->SendDebug("SendWallboxRequest", "Login fehlgeschlagen.", 0);
            return null;
        }

        // Debug Ausgabe des Kommandos
        $this->SendDebug(
            "SendWallboxRequest",
            "Sende an {$endpoint}: " . json_encode($data),
            0
        );
        ...
        if ($httpCode !== 200 || !$response) {
            $this->SendDebug(
                "SendWallboxRequest",
                "Fehler: HTTP={$httpCode}, Antwort=" . substr((string)$response, 0, 400),
                0
            );
            return null;
        }
        ...
        if (!isset($decoded['code']) || $decoded['code'] !== "0") {
            $this->SendDebug(
                "SendWallboxRequest",
                "API-Antwort Fehler: " . json_encode($decoded),
                0
            );
            return null;
        }

        $this->SendDebug("SendWallboxRequest", "API-Antwort OK", 0);
        return $decoded;
    }
    ...
    private function SetValueIfChanged(string $ident, $value): bool
    {
